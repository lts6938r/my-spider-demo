...
func okHandle(r *Request) (*Response, error) {
	return &Response{StatusCode: http.StatusOK, Request: r}, nil
}

func failHandle(*Request) (*Response, error) {
...
func TestUserAgentMiddleware(t *testing.T) {
	...
	r, _ := NewRequest("", "https://example.com/a", nil)
	h(r)
	if got := r.Header.Get("User-Agent"); got != "test-ua" {
		t.Errorf("未设置 UA 时应补默认, got %q", got)
	}

	r2, _ := NewRequest("", "https://example.com/b", nil)
	...
	r3 := &Request{URL: "https://example.com/c"}
...
func TestLoggingMiddleware(t *testing.T) {
	...
	r, _ := NewRequest("", "https://example.com/x", nil)
	r.ID = 7
	h(r)
