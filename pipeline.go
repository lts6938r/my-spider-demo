...
// Pipeline 处理解析产出的 Item（清洗、校验、持久化……）。
//
// Item 是 map 引用，Pipeline 可以就地增删改字段；
...
type PipelineFunc func(item Item) error

func (f PipelineFunc) ProcessItem(item Item) error { return f(item) }

// ConsolePipeline 把每个 Item 序列化成一行 JSON 打印到 stdout。
// JSONL 格式天然可重定向到文件：go run ./cmd/quotes > out.jsonl
type ConsolePipeline struct{}

func (ConsolePipeline) ProcessItem(item Item) error {
...
// JSONLFilePipeline 把每个 Item 以 JSON 行追加写入文件（jsonl：
...
func (p *JSONLFilePipeline) ProcessItem(item Item) error {
	p.mu.Lock()
	defer p.mu.Unlock()
	if p.closed {
