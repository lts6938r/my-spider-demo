...
// 组装失败应在运行前暴露而非运行中崩溃。
func NewEngineFromConfig(cfg *Config, duper Duper, pipes ...Pipeline) (*Engine, error) {
	if cfg == nil {
		cfg = DefaultConfig()
	}
	...
	var mws []Middleware
	mws = append(mws, LoggingMiddleware(lg))
	if cfg.MaxAttempts > 1 {
		mws = append(mws, RetryMiddleware(RetryConfig{
	...
	if cfg.GlobalQPS > 0 || len(cfg.DomainQPS) > 0 {
		mws = append(mws, RateLimitMiddleware(RateLimitConfig{
			GlobalQPS: cfg.GlobalQPS,
			DomainQPS: cfg.DomainQPS,
		}))
	}
	if cfg.UserAgent != "" {
		mws = append(mws, UserAgentMiddleware(cfg.UserAgent))
	}

	// 下载器可替换：HTTP（net/http）或 CDP（真实浏览器），引擎无感知
	var down Downloader = NewHTTPDownloader(cfg.Timeout.Duration)
	if strings.EqualFold(cfg.Downloader, "cdp") {
	...
	var sched Scheduler = NewFIFOScheduler()
	...
	if cfg.Output != "" {
		pipes = append(pipes, NewJSONLFilePipeline(cfg.Output))
	}
	// SQLite 非空：追加 SQLite 管道（打开失败立即报错）
	if cfg.SQLite != "" {
		sqlp, err := NewSQLitePipeline(cfg.SQLite, cfg.SQLiteTable)
		if err != nil {
			return nil, err
		}
