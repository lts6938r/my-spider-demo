...
func TestPipelineFuncAdapter(t *testing.T) {
	var got spider.Item
	fn := PipelineFunc(func(it spider.Item) error {
		got = it
		return nil
	})
	if err := fn.ProcessItem(spider.Item{"a": 1}); err != nil {
		t.Fatal(err)
	}
	if got["a"] != 1 {
...
func TestConsolePipelineOutput(t *testing.T) {
	...
	err = ConsolePipeline{}.ProcessItem(spider.Item{"k": "v", "n": 1})
...
func TestJSONLFilePipeline(t *testing.T) {
	...
	if err := p.ProcessItem(spider.Item{"id": 1}); err != nil {
		t.Fatal(err)
	}
	if err := p.ProcessItem(spider.Item{"id": 2}); err != nil {
		t.Fatal(err)
	}
	...
	if err := p.ProcessItem(spider.Item{"id": 3}); err == nil {
...
func TestJSONLFilePipelineAppends(t *testing.T) {
	...
	p1.ProcessItem(spider.Item{"run": 1})
	p1.Close()

	p2 := NewJSONLFilePipeline(path)
	p2.ProcessItem(spider.Item{"run": 2})
	p2.Close()
