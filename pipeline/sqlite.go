...
// SQLitePipeline 把 spider.Item 持久化到 SQLite（modernc.org/sqlite，纯 Go 实现，
// 无需 cgo——本机没有 gcc 也能编译；代价是比 cgo 版略慢，对爬虫写入量足够）。
//
// Schema 取舍：spider.Item 是无 schema 的 map，采用「一行一 item 的 JSON 文档」
...
func (p *SQLitePipeline) ProcessItem(item spider.Item) error {
	p.mu.Lock()
	defer p.mu.Unlock()
	if p.closed {
