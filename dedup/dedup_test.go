...
func TestHashSetDuperFingerprint(t *testing.T) {
	...
	r1, _ := spider.NewRequest("", "https://example.com/p?a=1&b=2", nil)
	r2, _ := spider.NewRequest("", "https://example.com/p?b=2&a=1", nil) // query 顺序不同
	r3, _ := spider.NewRequest("", "https://example.com/q?a=1&b=2", nil) // path 不同
	r4, _ := spider.NewRequest("POST", "https://example.com/p?a=1&b=2", nil)
...
func TestHashSetDuperConcurrent(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		...
		go func() {
			defer wg.Done()
			r, err := spider.NewRequest("", "https://example.com/same", nil)
			if err != nil {
				t.Error(err)
				return
			}
