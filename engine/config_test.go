...
func TestDefaultConfig(t *testing.T) {
	cfg := DefaultConfig()
	if cfg.UserAgent != downloader.DefaultUserAgent || cfg.Timeout.Duration != 30*time.Second {
...
func TestNewEngineFromConfigAssemblesChain(t *testing.T) {
	...
	eng, err := NewEngineFromConfig(cfg, dedup.NewHashSetDuper(), pipeline.ConsolePipeline{})
	if err != nil {
		t.Fatal(err)
	}
	...
	// SaveDir 已开启：Logging + Save + Retry + RateLimit + UA
	if len(eng.Mws) != 5 {
		t.Errorf("Mws 数量 = %d, want 5", len(eng.Mws))
	}
	if eng.Workers != 3 {
		t.Errorf("Workers = %d, want 3（配置传导）", eng.Workers)
	}
	if _, ok := eng.Sched.(*scheduler.PriorityQueueScheduler); !ok {
		t.Errorf("Sched 类型 = %T, want *scheduler.PriorityQueueScheduler（配置选择）", eng.Sched)
	}
	if _, ok := eng.Down.(*downloader.HTTPDownloader); !ok {
		t.Errorf("Down 类型 = %T, want *downloader.HTTPDownloader（默认）", eng.Down)
	}
	...
	if len(eng2.Mws) != 3 { // Logging + Retry + UA
