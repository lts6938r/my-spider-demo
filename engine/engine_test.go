...
// testLogger 把日志全部丢弃，让测试输出干净（middleware 包有同名副本，
// 跨包不值得为此引入 testutil 包）。
...
func (s spiderStub) StartRequests() []*spider.Request { return s.reqs }
...
// 并演示 Meta 沿请求链传递。
func jsonParse(resp *spider.Response) spider.Result {
	var p testPage
	if err := json.Unmarshal(resp.Body, &p); err != nil {
		return spider.Result{Err: fmt.Errorf("decode %s: %w", resp.Request.URL, err)}
	}

	res := spider.Result{}
	for _, it := range p.Items {
		res.Items = append(res.Items, spider.Item{"id": it.ID, "page": resp.Request.Meta["page"]})
	}
	for _, link := range p.Links {
		nr, err := spider.NewRequest("", link, jsonParse)
		if err != nil {
...
func captureEngine(duper dedup.Duper, mws ...middleware.Middleware) (*Engine, *[]spider.Item) {
	got := &[]spider.Item{}
	return &Engine{
...
func TestEngineEndToEnd(t *testing.T) {
	...
	start, err := spider.NewRequest("", srv.URL+"/p1", jsonParse)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := eng.Run(spiderStub{"demo", []*spider.Request{start}}); err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestEngineParseErrorDropsResult(t *testing.T) {
	...
	boom := func(*spider.Response) spider.Result {
		// 模拟"页面结构不符合预期"：带着半成品 item 报错
		return spider.Result{Items: []spider.Item{{"bad": true}}, Err: errors.New("unexpected layout")}
	}
	start, _ := spider.NewRequest("", srv.URL, boom)

	eng, got := captureEngine(nil)
	if err := eng.Run(spiderStub{"boom", []*spider.Request{start}}); err != nil {
		t.Fatalf("Run: %v", err)
	}
	if len(*got) != 0 {
...
func TestEngineDownloadFailureContinues(t *testing.T) {
	...
	goodReq, _ := spider.NewRequest("", good.URL, jsonParse)
	badReq, _ := spider.NewRequest("", bad.URL, jsonParse)
	eng, got := captureEngine(nil)
	if err := eng.Run(spiderStub{"mixed", []*spider.Request{badReq, goodReq}}); err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestEnginePipelineStopsOnError(t *testing.T) {
	...
	reject := pipeline.PipelineFunc(func(spider.Item) error { return errors.New("invalid") })
	var secondSaw int
	count := pipeline.PipelineFunc(func(spider.Item) error { secondSaw++; return nil })

	start, _ := spider.NewRequest("", srv.URL, jsonParse)
	eng, _ := captureEngine(nil)
	eng.Pipes = []pipeline.Pipeline{reject, count}
	if err := eng.Run(spiderStub{"pipe", []*spider.Request{start}}); err != nil {
		t.Fatalf("Run: %v", err)
	}
	if secondSaw != 0 {
...
func TestEngineDedup(t *testing.T) {
	...
	start, _ := spider.NewRequest("", srv.URL+"/start", jsonParse)
	eng, got := captureEngine(dedup.NewHashSetDuper())
	if err := eng.Run(spiderStub{"dedup", []*spider.Request{start}}); err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestEngineWithRetryMiddleware(t *testing.T) {
	...
	start, _ := spider.NewRequest("", srv.URL, jsonParse)
	cfg := middleware.RetryConfig{MaxAttempts: 3, BaseDelay: time.Millisecond, MaxDelay: 2 * time.Millisecond}
	eng, got := captureEngine(nil, middleware.RetryMiddleware(cfg, testLogger))
	if err := eng.Run(spiderStub{"retry", []*spider.Request{start}}); err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestEngineConcurrentDownloads(t *testing.T) {
	...
	for i := 0; i < 12; i++ {
		r, err := spider.NewRequest("", fmt.Sprintf("%s/%d", srv.URL, i), nil)
		if err != nil {
			t.Fatal(err)
		}
		reqs = append(reqs, r)
	...
	if eng.Downloaded != 12 || eng.Failed != 0 {
		t.Errorf("Downloaded=%d Failed=%d, want 12/0", eng.Downloaded, eng.Failed)
	}
	if m := maxC.Load(); m < 2 {
	...
	// 12 × 150ms 任务（超过两轮派发，覆盖多轮投喂路径）：串行 1.8s，4 worker 理论 ~450ms
...
func TestEngineConcurrentDedup(t *testing.T) {
	...
	eng := &Engine{
		...
		Down:    downloader.NewHTTPDownloader(5 * time.Second),
		Workers: 8,
		Duper:   dedup.NewHashSetDuper(),
		Logger:  testLogger,
		Pipes:   []pipeline.Pipeline{pipeline.PipelineFunc(func(spider.Item) error { return nil })},
	}
	if err := eng.Run(spiderStub{"dedup2", []*spider.Request{start}}); err != nil {
		t.Fatal(err)
	}
...
func TestEngineGracefulShutdown(t *testing.T) {
	...
	eng := &Engine{
		Sched:   scheduler.NewFIFOScheduler(),
		Down:    downloader.NewHTTPDownloader(10 * time.Second),
		Workers: 4,
		Logger:  testLogger,
	}
	start := time.Now()
...
func TestRunIsUncancellableEquivalent(t *testing.T) {
	...
	start, _ := spider.NewRequest("", srv.URL, jsonParse)
	eng, got := captureEngine(nil)
	if err := eng.Run(spiderStub{"plain", []*spider.Request{start}}); err != nil {
		t.Fatalf("Run: %v", err)
	}
	if len(*got) != 1 {
		t.Errorf("got %v", *got)
...
func TestEnginePriorityScheduling(t *testing.T) {
	...
	low, _ := spider.NewRequest("", srv.URL+"/low", nil)
	high, _ := spider.NewRequest("", srv.URL+"/high", nil)
	high.Priority = 10
	...
	if err := eng.Run(spiderStub{"prio", []*spider.Request{low, high}}); err != nil {
		t.Fatal(err)
	}
...
func TestEngineShutdownPersistsCompletedItems(t *testing.T) {
	...
	fast, _ := spider.NewRequest("", srv.URL+"/fast", jsonParse)
	slow, _ := spider.NewRequest("", srv.URL+"/slow", jsonParse)
	...
	eng := &Engine{
		...
		Down:    downloader.NewHTTPDownloader(10 * time.Second),
		Workers: 2,
		Pipes:   []pipeline.Pipeline{pipeline.NewJSONLFilePipeline(path)},
		Logger:  testLogger,
	}
	if err := eng.RunContext(ctx, spiderStub{"persist", []*spider.Request{fast, slow}}); !errors.Is(err, context.Canceled) {
