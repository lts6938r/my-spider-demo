...
func mkReq(id int) *spider.Request {
	r, err := spider.NewRequest("", fmt.Sprintf("https://example.com/item/%d", id), nil)
	if err != nil {
		panic(err)
	}
	return r
...
func prioReq(id, prio int) *spider.Request {
	r := mkReq(id)
	r.Priority = prio
	return r
}
...
func TestPrioritySchedulerOrder(t *testing.T) {
	...
	for _, r := range []*spider.Request{prioReq(1, 0), prioReq(2, 10), prioReq(3, 5), prioReq(4, 10)} {
		s.Push(r)
	}
	if s.Len() != 4 {
